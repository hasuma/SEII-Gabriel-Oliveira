...
simulationType = 'manual' #  automatic/manual
...
yvals = [0.0, 0.0, 0.0]  # a list of desired y pos, vel and acc
zvals = [0.0, 0.0, 0.0]  # a list of desired z pos, vel and acc
dyvals = [0.01, 0.0, 0.0] # a list of desired y increments pos, vel and acc when pressing a key
dzvals = [0.01, 0.0, 0.0] # a list of desired z increments pos, vel and acc when pressing a key
# drawing stuff
# pixels per meter
PPM = 200.0
...
label_button = create_label()
labels = [label_pos, label_ang, label_force, label_time, label_traj, label_button]
...
def simulate(_):
    ...
    global currtime, actual_trajectory, traj_index, zvals, yvals
    currtime += dt
    ...
    # Control calculate in continuous time - CHANGE FOR DISCRETE TIME
...
def update_state_label(_):
    ...
    if simulationType == 'automatic':
        ...
        label_traj.text = 'Manual Trajectory'
        i = 0
        text = ''
        for key in keypressed:
            ...
            i += 1
        if text == '':
            text = 'No control key pressed'
        else:
            text = text + ' key(s) pressed'
        label_button.text = 'Control Key: {}'.format(text)
...
class MyWindow(pyglet.window.Window):
    def __init__(self):
        ...
        self.set_caption('Quadcopter Simulator')
    ...
    def update(self, _):
        global yvals, zvals, keypressed
        if simulationType == 'manual':
            keypressed = [0, 0, 0, 0]
            if self.key_handler[key.UP]:
                zvals = np.add(zvals,dzvals)
                keypressed[0] = 1
            if self.key_handler[key.DOWN]:
                zvals = np.add(zvals, np.multiply(-1,dzvals))
                keypressed[1] = 1
            if self.key_handler[key.RIGHT]:
                yvals = np.add(yvals, dyvals)
                keypressed[2] = 1
            if self.key_handler[key.LEFT]:
                yvals = np.add(yvals, np.multiply(-1, dyvals))
                keypressed[3] = 1
...
if __name__ == '__main__':
    ...
    pyglet.clock.schedule_interval(update_state_label, DT)
    pyglet.clock.schedule_interval(record_curr_pos, DT)
